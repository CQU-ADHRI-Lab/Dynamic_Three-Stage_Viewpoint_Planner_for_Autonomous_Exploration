...
/**
 * @brief 获取在指定边界框内的未知点云，并检测其中的前沿点
 * @param center 边界框的中心
 * @param bounding_box_size 边界框的大小
 * 遍历边界框，寻找边界点
 */
...
void DualStateFrontier::getUnknowPointcloudInBoundingBox(const StateVec& center, const StateVec& bounding_box_size)
{
  ...
  const double epsilon = 0.001;  // Small offset to not hit boundary of nodes.  // 设置一个小的偏移量，以避免撞到节点的边界
  StateVec epsilon_3d;
...
{//检查点 point 是否在 boundary_polygon_ 多边形内。如果点不在多边形内，函数返回 false。
  if (boundaryLoaded_)
  {
...
//检查一个给定的三维点是否在已清理的前沿点集合中。已清理的前沿点集合是一个 pcl::PointXYZ 类型的点云，其中包含了已经被清理过的前沿点。
...
//检查一个给定的三维点是否在传感器的范围内，并且是否在图点的搜索半径内。如果点在范围内，并且没有被障碍物占据，函数返回 true，否则返回 false。
bool DualStateFrontier::inSensorRangeofGraphPoints(StateVec point)
{
  ...
  if (graphPoints_->points.size() > 0)
  {
   ...
   // 遍历搜索到的点的索引和距离，对于每个点，计算当前点 point 到搜索到的点的欧几里得距离。
    for (int i = 0; i < pointSearchInd.size(); i++)
    {
      ...
      //如果计算得到的距离小于 kSearchRadius，则认为当前点在传感器的范围内，并且在图点的搜索半径内。
      double rangeSq = pow(kSearchRadius, 2.0);
      /*dir.transpose() 这一部分调用了 transpose 方法，该方法可能是 StateVec 类的一个成员函数，用于返回向量的转置。在三维向量的情况下，转置操作通常不会改变向量的数值，因为三维向量的转置仍然是它本身。但是，如果 StateVec 类实现了转置方法，那么它可能会返回一个不同的对象，这个对象可能具有不同的内部表示或行为。

接下来，.dot(dir) 调用了 dot 方法，该方法可能是 StateVec 类的另一个成员函数，用于计算两个向量的点积（dot product）。点积是两个向量的对应分量相乘后求和的结果。*/
      ...
      //如果点在传感器的范围内，并且在图点的搜索半径内，并且在传感器的垂直视野内，则检查该点是否被障碍物占据。如果点没有被障碍物占据，则函数返回 true。
...
//检查一个给定的三维点是否在机器人的传感器范围内。如果点在范围内，并且没有被障碍物占据，函数返回 true，否则返回 false。
...
//获取机器人周围的边界点云，更新局部边界点云,更新全局边界点云,全局边界点云的邻域,并将其发布到相应的主题上。
...
//将未知点云、全局边界点云和局部边界点云转换为 ROS 消息格式，并发布到相应的主题上。
...
//处理接收到的里程计消息和地形点云消息，并更新机器人的位置和地形信息。
...
//处理接收到的点云消息，并将其转换为 PCL 格式的点云数据，然后存储在 graphPoints_ 成员变量中。
...
bool DualStateFrontier::initialize()
{
  ...
  {
    //定时器 executeTimer_，它会以 kExecuteFrequency_ 定义的频率触发 DualStateFrontier::execute 成员函数。nh_private_ 是一个 ros::NodeHandle 对象，用于在私有命名空间中创建定时器。ros::Duration(1.0 / kExecuteFrequency_) 指定了定时器的周期，即每 1.0 / kExecuteFrequency_ 秒触发一次。&DualStateFrontier::execute 是成员函数的指针，this 表示定时器将在当前 DualStateFrontier 类的实例上调用该成员函数。
